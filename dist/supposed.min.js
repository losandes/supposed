...
module.exports={name:"makeSuiteConfig",factory:function factory(dependencies){'use strict';var envvars=dependencies.envvars,pubsub=dependencies.pubsub,reporterFactory=dependencies.reporterFactory;var makeSuiteId=function makeSuiteId(){return"S".concat((Math.random()*16777215<<0).toString(16).toUpperCase())};var maybeOverrideValueFactory=function maybeOverrideValueFactory(suiteConfig,options){return function(name,factory){var value=factory(options);if(typeof value!=="undefined"){suiteConfig[name]=value}}};var addValues=function addValues(suiteConfig,options){var maybeOverrideValue=maybeOverrideValueFactory(suiteConfig,_objectSpread({},options));maybeOverrideValue("assertionLibrary",function(options){return options.assertionLibrary});maybeOverrideValue("name",function(options){return typeof options.name==="string"&&options.name.trim().length?options.name.trim():undefined});maybeOverrideValue("timeout",function(options){return typeof options.timeout==="number"&&options.timeout>0?options.timeout:undefined});maybeOverrideValue("exit",function(options){return typeof options.exit==="function"?options.exit:undefined});maybeOverrideValue("useColors",function(options){return typeof options.useColors==="boolean"?options.useColors:undefined});maybeOverrideValue("timeUnits",function(options){return typeof options.timeUnits==="string"&&options.timeUnits.trim().length?options.timeUnits.trim().toLowerCase():undefined});maybeOverrideValue("reportOrder",function(options){return typeof options.reportOrder==="string"&&["deterministic","non-deterministic"].indexOf(options.reportOrder.trim().toLowerCase())?options.reportOrder.trim().toLowerCase():undefined});maybeOverrideValue("match",function(options){if(typeof options.match==="string"){return new RegExp(options.match)}else if(options.match&&typeof options.match.test==="function"){return options.match}else if(options.match===null){// let hard coded options override (I use this in the tests)
return options.match}});maybeOverrideValue("givenSynonyms",function(options){if(Array.isArray(options.givenSynonyms)){var synonyms=options.givenSynonyms.filter(function(synonym){return typeof synonym==="string"&&synonym.trim().length}).map(function(synonym){return synonym.trim()});if(synonyms.length){return synonyms}}});maybeOverrideValue("whenSynonyms",function(options){if(Array.isArray(options.whenSynonyms)){var synonyms=options.whenSynonyms.filter(function(synonym){return typeof synonym==="string"&&synonym.trim().length}).map(function(synonym){return synonym.trim()});if(synonyms.length){return synonyms}}})};var addReporters=function addReporters(suiteConfig,options){var maybeOverrideValue=maybeOverrideValueFactory(suiteConfig,_objectSpread({},options));maybeOverrideValue("reporters",function(options){var reporters=[];var makeReporterArray=function makeReporterArray(input){return input.split(",").map(function(reporter){return reporter.trim().toUpperCase()})};var addReporter=function addReporter(nameOrFunc){if(typeof nameOrFunc==="string"){var reporter=reporterFactory.get(nameOrFunc);reporters.push(reporter)}else{reporterFactory.add(nameOrFunc);addReporter(nameOrFunc.name)}};// reporter: accept strings, functions, or objects
...
/**
     ...
     *     tests: IAsyncTest[]
     *   }
     */var mapper=function mapper(config,makeBatch,makeBatchId,byMatcher){return function(batch){var theories=makeBatch(batch).filter(byMatcher);return{batchId:theories.length?theories[0].id:makeBatchId(),theories:theories}}};var isPlan=function isPlan(maybePlan){return typeof maybePlan!=="undefined"&&maybePlan!==null&&typeof maybePlan.count==="number"&&typeof maybePlan.completed==="number"&&Array.isArray(maybePlan.batches)&&Array.isArray(maybePlan.order)};var planner=function planner(config,mapToBatch){var makeEmptyPlan=function makeEmptyPlan(){return{count:0,completed:0,batches:[],order:[]}};var plan=makeEmptyPlan();var addToPlan=function addToPlan(description,assertions){return normalizeBatch(description,assertions).then(mapToBatch).then(function(context){if(context.theories.length){plan.batches.push(context);context.theories.forEach(function(theory){theory.assertions.forEach(function(assertion){return plan.order.push(assertion.id)})});plan.count=plan.order.length}return plan})};addToPlan.getPlan=function(){return plan};addToPlan.resetPlan=function(){plan=makeEmptyPlan()};return addToPlan};var brokenTestPublisher=function brokenTestPublisher(suiteId){return function(error){return pubsub.publish({type:TestEvent.types.TEST,status:TestEvent.status.BROKEN,behavior:"Failed to load test: ".concat(error.filePath),suiteId:suiteId,error:error})}};/**
     ...
     */var toOneArray=function toOneArray(prop){return function(output,current){return Array.isArray(current[prop])?output.concat(current[prop]):output.concat([current[prop]])}};var fullfilledToOneArray=toOneArray("value");var failedToOneArray=toOneArray("reason");var batchRunner=function batchRunner(config,publishOneBrokenTest){return function(batch,plan){return pubsub.publish({type:TestEvent.types.START_BATCH,batchId:batch.batchId,suiteId:config.name}).then(function(){// map the batch theories to tests
return batch.theories.map(function(theory){return new AsyncTest(theory,config.makeTheoryConfig(theory),batch.batchId,config.name)})}).then(function(tests){return allSettled(tests.map(function(test){return test()}))}).then(function(results){return{results:results.filter(function(result){return result.status==="fullfilled"}).reduce(fullfilledToOneArray,[]),broken:results.filter(function(result){return result.status!=="fullfilled"}).reduce(failedToOneArray,[]),batchTotals:Tally.getTally().batches[batch.batchId]}}).then(function(context){var publishEndBatch=function publishEndBatch(){return pubsub.publish({type:TestEvent.types.END_BATCH,batchId:batch.batchId,suiteId:config.name,totals:context.batchTotals})};if(Array.isArray(context.broken)&&context.broken.length){// these tests failed during the planning stage
...
var tester=function tester(config,runBatch,runnerMode){return function(plan){if(!runnerMode){config.registerReporters()}return Promise.resolve({plan:plan}).then(function(context){if(!runnerMode){return pubsub.publish({type:TestEvent.types.START,suiteId:config.name,plan:context.plan}).then(function(){return context})}return Promise.resolve(context)}).then(function(context){return Promise.all(context.plan.batches.map(function(batch){return runBatch(batch,context.plan)}))}).then(function(context){if(!runnerMode){return pubsub.publish({type:TestEvent.types.END_TALLY,suiteId:config.name}).then(function(){return pubsub.publish({type:TestEvent.types.END,suiteId:config.name,totals:Tally.getSimpleTally(),plan:plan})}).then(function(){return context})}return Promise.resolve(context)}).then(function(context){if(Array.isArray(context)&&context.length===1){return context[0]}return context}).catch(function(e){pubsub.publish({type:TestEvent.types.TEST,status:TestEvent.status.BROKEN,behavior:"Failed to load test",suiteId:config.name,error:e});throw e})}};var runner=function runner(config,suite,publishOneBrokenTest,execute){return function(planContext){var plan=planContext.plan,files=planContext.files,broken=planContext.broken;config.registerReporters();return pubsub.publish({type:TestEvent.types.START,suiteId:config.name,plan:plan}).then(function(){if(broken&&broken.length){// these tests failed during the planning stage
...
return{output:output,tally:Tally.getSimpleTally()}}).then(function(context){plan.completed=context.tally.total;return pubsub.publish({type:TestEvent.types.END,suiteId:config.name,totals:context.tally,plan:plan}).then(function(){return context});// pass through
    ...
    */function Suite(suiteConfig,envvars){var runnerMode=false;suiteConfig=_objectSpread({},suiteConfig);/**
       ...
       */var configure=function configure(suiteDotConfigureOptions){suiteDotConfigureOptions=_objectSpread({},suiteDotConfigureOptions);var _suiteConfig=Object.keys(suiteConfig).concat(Object.keys(suiteDotConfigureOptions)).reduce(function(cfg,key){cfg[key]=typeof suiteDotConfigureOptions[key]!=="undefined"?suiteDotConfigureOptions[key]:suiteConfig[key];return cfg},{});pubsub.reset();pubsub.subscribe(reporterFactory.get(Tally.name));var config=makeSuiteConfig(_suiteConfig);var publishOneBrokenTest=brokenTestPublisher(config.name);var _ref2=new BatchComposer(config),makeBatch=_ref2.makeBatch,makeBatchId=_ref2.makeBatchId;var byMatcher=matcher(config);var mapToBatch=mapper(config,makeBatch,makeBatchId,byMatcher);var runBatch=batchRunner(config,publishOneBrokenTest);var plan=planner(config,mapToBatch);var test=function test(description,assertions){if(runnerMode){return plan(description,assertions)}else{return planner(config,mapToBatch)(description,assertions)// new planner for each test execution
.then(tester(config,runBatch,runnerMode))}};test.id=config.name;test.plan=plan;test.config=config;test.dependencies=_suiteConfig&&_suiteConfig.inject;test.configure=configure;test.subscribe=function(subscription){pubsub.subscribe(subscription);return test};test.runner=function(options){options=options||{};if(envvars&&envvars.file&&typeof envvars.file.test==="function"){options.matchesNamingConvention=envvars.file}var findAndStart=browserRunner(config,test);var addPlanToContext=function addPlanToContext(){return function(context){context.plan=plan.getPlan();return context}};var findAndPlan=function findAndPlan(){runnerMode=true;return findFiles(options).then(resolveTests()).then(makePlans(test)).then(addPlanToContext())};var _run=function run(){return runner(config,test,publishOneBrokenTest,tester(config,runBatch,runnerMode))};return{plan:findAndPlan,// find and run (node)
run:function run(){return findAndPlan().then(_run()).then(config.exit)},// run (browser|node)
runTests:function runTests(planOrTests){runnerMode=true;if(planOrTests&&isPlan(planOrTests.plan)){return Promise.resolve(planOrTests).then(_run()).then(config.exit)}if(Array.isArray(planOrTests)){options.tests=planOrTests}else if(typeof planOrTests==="function"){options.tests=planOrTests()}return makePlans(test)(options).then(addPlanToContext()).then(_run()).then(config.exit)},// start test server (browser)
startServer:findAndStart(options)}};// @deprecated - may go away in the future
test.printSummary=function(){return pubsub.publish({type:TestEvent.types.END,suiteId:config.name,totals:Tally.getSimpleTally()})};// @deprecated - may go away in the future
...
}).write}}).add(function SummaryReporter(){return{write:ConsoleReporter({formatter:module.factories.SummaryFormatterFactory({consoleStyles:consoleStyles,DefaultFormatter:DefaultFormatter,TestEvent:TestEvent}).SummaryFormatter()}).write}}).add(function TapReporter(){return{write:ConsoleReporter({formatter:module.factories.TapFormatterFactory({consoleStyles:consoleStyles,TestEvent:TestEvent}).TapFormatter()}).write}});var _module$factories$Asy=module.factories.AsyncTestFactory({isPromise:isPromise,pubsub:pubsub,TestEvent:TestEvent,clock:clock,duration:duration,addDurations:time.addDurations}),AsyncTest=_module$factories$Asy.AsyncTest;var _module$factories$has=module.factories.hashFactory(),hash=_module$factories$has.hash;var _module$factories$mak2=module.factories.makeBatchFactory({hash:hash}),BatchComposer=_module$factories$mak2.BatchComposer;var _module$factories$mak3=module.factories.makeSuiteConfigFactory({envvars:envvars,pubsub:pubsub,reporterFactory:reporterFactory}),makeSuiteConfig=_module$factories$mak3.makeSuiteConfig;var _module$factories$Sui=module.factories.SuiteFactory({allSettled:allSettled,AsyncTest:AsyncTest,BatchComposer:BatchComposer,makeSuiteConfig:makeSuiteConfig,pubsub:pubsub,reporterFactory:reporterFactory,makePlans:makePlans,Tally:Tally,TestEvent:TestEvent}),Suite=_module$factories$Sui.Suite;var suite=new Suite(options);suite.Suite=Supposed;if(!suites[suite.config.name]){suites[suite.config.name]=suite}return suite}supposed=Supposed({name:"supposed"});suites.supposed=supposed;supposed.suites=suites;supposed.time=time;window.supposed=supposed})(window);