...
function maybeLog(result){return result&&typeof result.log!=="undefined"?result.log:undefined}function maybeContext(result){return result&&typeof result.context!=="undefined"?result.context:undefined}/**
     ...
     * @param {Object} context
     */function assertOne(batchId,assertion,test){var pass=function pass(result){return publish({type:TestEvent.types.TEST,status:TestEvent.status.PASSED,batchId:batchId,testId:assertion.id,behavior:assertion.behavior,log:maybeLog(result),context:maybeContext(result)})};var fail=function fail(e){return publish({type:TestEvent.types.TEST,status:TestEvent.status.FAILED,batchId:batchId,testId:assertion.id,behavior:assertion.behavior,error:e})};try{if(assertion.skipped){return publish({type:TestEvent.types.TEST,status:TestEvent.status.SKIPPED,batchId:batchId,testId:assertion.id,behavior:assertion.behavior})}return publish({type:TestEvent.types.START_TEST,batchId:batchId,testId:assertion.id,behavior:assertion.behavior}).then(function(){return test()}).then(pass).catch(fail).finally(function(context){return publish({type:TestEvent.types.END_TEST,batchId:batchId,testId:assertion.id,behavior:assertion.behavior}).then(function(){return context})})}catch(e){return fail(e)}}// /assertOne
/**
...
return{DefaultFormatter:DefaultFormatter}}};module.exports={name:"JsonFormatter",factory:function factory(dependencies){'use strict';var TestEvent=dependencies.TestEvent;function JsonFormatter(){var format=function format(event){if(event.type===TestEvent.types.START){return"[".concat(JSON.stringify({event:event},null,2),",")}else if(event.type===TestEvent.types.END){return"".concat(JSON.stringify({event:event},null,2),"]")}else if([TestEvent.types.END_TALLY].indexOf(event.type)===-1){return"".concat(JSON.stringify({event:event},null,2),",")}};return{format:format}}return{JsonFormatter:JsonFormatter}}};module.exports={name:"MarkdownFormatter",factory:function factory(dependencies){'use strict';var consoleStyles=dependencies.consoleStyles,TestEvent=dependencies.TestEvent;var newLine=consoleStyles.newLine();var addToMd=function addToMd(parts,md){if(!parts.length){return}var part=parts.shift().trim();md[part]=md[part]||{};if(parts.length){addToMd(parts,md[part])}};var space="                                                               ";var toBullets=function toBullets(md,layer){if(typeof layer==="undefined")layer=0;var output="";Object.keys(md).forEach(function(key){var line=key.replace(/\n/g,newLine+space.substring(0,(layer+1)*2));output+=space.substring(0,layer*2)+"* ".concat(line).concat(newLine);output+=toBullets(md[key],layer+1)});return output};var totals=function totals(_totals){return"## Totals".concat(newLine)+"".concat(newLine,"- **total**: ").concat(_totals.total)+"".concat(newLine,"- **passed**: ").concat(_totals.passed)+"".concat(newLine,"- **failed**: ").concat(_totals.failed)+"".concat(newLine,"- **skipped**: ").concat(_totals.skipped)+"".concat(newLine,"- **duration**: ").concat((_totals.endTime-_totals.startTime)/1000,"s")};function MarkdownFormatter(){var title="Tests";var md={};var format=function format(event){if(event.type===TestEvent.types.START){title=event.suiteId}else if(event.type===TestEvent.types.END){return"# ".concat(title).concat(newLine).concat(newLine).concat(toBullets(md)).concat(newLine).concat(totals(event.totals))}else if(event.type===TestEvent.types.INFO){// should we include info?
// return `# ${event.behavior}${formatInfo(event.behavior, event.log, 'comment')}`
}else if(event.type===TestEvent.types.TEST){addToMd(event.behavior.split(","),md)}};// /format
return{format:format}}// /Formatter
...
if([TestEvent.types.START,TestEvent.types.TEST,TestEvent.types.INFO,TestEvent.types.END].indexOf(event.type)>-1){var line=format(event);if(!line){return}console.log(line);reportPre.append("".concat(line,"\n"));scrollToBottom()}};// /write
...
var totals={total:0,passed:0,skipped:0,failed:0,broken:0,startTime:-1,endTime:-1,results:[],batches:{}};var makeBatchTally=function makeBatchTally(event){if(totals.batches[event.batchId]){return publish({type:TestEvent.types.TEST,status:TestEvent.status.BROKEN,batchId:event.batchId,error:new Error("Duplicate Batch Ids were created, or multiple START_BATCH events were emitted for the same batch")}).then(function(){return undefined})}var tally=makeTally();tally.startTime=now();return Promise.resolve(tally)};var bump=function bump(event){try{var name=event.status.toLowerCase();totals[name]+=1;totals.total+=1;if(totals.batches[event.batchId]){totals.batches[event.batchId][name]+=1;totals.batches[event.batchId].total+=1}totals.results.push(event)}catch(e){console.log(event);console.log(e)}};function Tally(){var write=function write(event){switch(event.type){case TestEvent.types.START:totals.startTime=now();return Promise.resolve();case TestEvent.types.START_BATCH:return makeBatchTally(event).then(function(tally){if(tally){totals.batches[event.batchId]=tally}});case TestEvent.types.TEST:bump(event);return Promise.resolve();case TestEvent.types.END_BATCH:totals.batches[event.batchId].endTime=now();return Promise.resolve();case TestEvent.types.END_TALLY:totals.endTime=now();return Promise.resolve();}// /switch
};// /write
...
function Supposed(options){var _module$factories$all=module.factories.allSettledFactory({}),allSettled=_module$factories$all.allSettled;var _module$factories$run=module.factories.runTestsFactory({allSettled:allSettled}),runTests=_module$factories$run.runTests;var envvars={assertionLibrary:{},reporters:["DEFAULT"],useColors:true,timeUnits:"us"};var _module$factories$Tes=module.factories.TestEventFactory({clock:time.clock,timeUnits:envvars.timeUnits}),TestEvent=_module$factories$Tes.TestEvent;var _module$factories$pub=module.factories.pubsubFactory({allSettled:allSettled,isPromise:isPromise,TestEvent:TestEvent}),Pubsub=_module$factories$pub.Pubsub;var _ref3=new Pubsub,publish=_ref3.publish,subscribe=_ref3.subscribe,subscriptionExists=_ref3.subscriptionExists,allSubscriptions=_ref3.allSubscriptions,reset=_ref3.reset;var consoleStyles=module.factories.consoleStylesFactory({envvars:envvars}).consoleStyles;var _module$factories$Tal=module.factories.TallyFactory({publish:publish,TestEvent:TestEvent}),TallyFactory=_module$factories$Tal.TallyFactory;var _TallyFactory=TallyFactory(),Tally=_TallyFactory.Tally;var _module$factories$rep=module.factories.reporterFactoryFactory({}),ReporterFactory=_module$factories$rep.ReporterFactory;var reporterFactory=new ReporterFactory;var ArrayReporter=module.factories.ArrayReporterFactory({}).ArrayReporter;reporterFactory.add(ArrayReporter);reporterFactory.add(function QuietReporter(){// legacy
return{write:new ArrayReporter().write}});reporterFactory.add(module.factories.NoopReporterFactory({}).NoopReporter);reporterFactory.add(Tally);function DefaultFormatter(options){return module.factories.DefaultFormatterFactory({consoleStyles:consoleStyles,TestEvent:TestEvent,SYMBOLS:options.SYMBOLS}).DefaultFormatter()}function ConsoleReporter(options){return module.factories.DomReporterFactory({TestEvent:TestEvent,formatter:options.formatter}).DomReporter()}var symbolFormatter=module.factories.SymbolFormatterFactory({consoleStyles:consoleStyles,DefaultFormatter:DefaultFormatter}).SymbolFormatter();reporterFactory.add(function DefaultReporter(){return{write:ConsoleReporter({formatter:symbolFormatter}).write}}).add(function BlockReporter(){return{write:ConsoleReporter({formatter:module.factories.BlockFormatterFactory({consoleStyles:consoleStyles,DefaultFormatter:DefaultFormatter}).BlockFormatter()}).write}}).add(function BriefReporter(){return{write:ConsoleReporter({formatter:module.factories.BriefFormatterFactory({consoleStyles:consoleStyles,DefaultFormatter:DefaultFormatter,TestEvent:TestEvent}).BriefFormatter()}).write}}).add(function JsonReporter(){return{write:ConsoleReporter({formatter:module.factories.JsonFormatterFactory({TestEvent:TestEvent}).JsonFormatter()}).write}}).add(function MarkdownReporter(){return{write:ConsoleReporter({formatter:module.factories.MarkdownFormatterFactory({consoleStyles:consoleStyles,TestEvent:TestEvent}).MarkdownFormatter()}).write}}).add(function JustTheDescriptionsReporter(){return{write:ConsoleReporter({formatter:{format:function format(event){if(event.type===TestEvent.types.TEST){return symbolFormatter.format(event).split("\n")[0]}else{return symbolFormatter.format(event)}}}}).write}}).add(function SummaryReporter(){return{write:ConsoleReporter({formatter:module.factories.SummaryFormatterFactory({consoleStyles:consoleStyles,DefaultFormatter:DefaultFormatter,TestEvent:TestEvent}).SummaryFormatter()}).write}}).add(function TapReporter(){return{write:ConsoleReporter({formatter:module.factories.TapFormatterFactory({consoleStyles:consoleStyles,TestEvent:TestEvent}).TapFormatter()}).write}});var _module$factories$Asy=module.factories.AsyncTestFactory({isPromise:isPromise,publish:publish,TestEvent:TestEvent}),AsyncTest=_module$factories$Asy.AsyncTest;var _module$factories$has=module.factories.hashFactory(),hash=_module$factories$has.hash;var _module$factories$mak=module.factories.makeBatchFactory({hash:hash}),BatchComposer=_module$factories$mak.BatchComposer;var _module$factories$mak2=module.factories.makeSuiteConfigFactory({defaults:envvars,subscriptionExists:subscriptionExists,subscribe:subscribe,allSubscriptions:allSubscriptions,reporterFactory:reporterFactory}),makeSuiteConfig=_module$factories$mak2.makeSuiteConfig;var _module$factories$Sui=module.factories.SuiteFactory({allSettled:allSettled,AsyncTest:AsyncTest,BatchComposer:BatchComposer,makeSuiteConfig:makeSuiteConfig,publish:publish,subscribe:subscribe,clearSubscriptions:reset,reporterFactory:reporterFactory,runTests:runTests,Tally:Tally,TestEvent:TestEvent}),Suite=_module$factories$Sui.Suite;var suite=new Suite(options);suite.Suite=Supposed;if(!suites[suite.config.name]){suites[suite.config.name]=suite}return suite}supposed=Supposed({name:"supposed"});suites.supposed=supposed;supposed.suites=suites;supposed.time=time;window.supposed=supposed})(window);