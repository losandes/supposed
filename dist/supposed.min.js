...
resolve({status:"rejected",reason:err})}})}))}return{allSettled:allSettled}}};module.exports={name:"AsyncTest",factory:function factory(dependencies){'use strict';var isPromise=dependencies.isPromise,publish=dependencies.publish,TestEvent=dependencies.TestEvent,clock=dependencies.clock,duration=dependencies.duration;function noop(){}/**
...
function maybeLog(result){return result&&typeof result.log!=="undefined"?result.log:undefined}function maybeContext(result){return result&&typeof result.context!=="undefined"?result.context:undefined}/**
     ...
     * @param {Object} context
     */function assertOne(batchId,assertion,test){var pass=function pass(startTime){return function(result){var endTime=clock();return publish({type:TestEvent.types.TEST,status:TestEvent.status.PASSED,batchId:batchId,testId:assertion.id,behavior:assertion.behavior,time:endTime,duration:duration(startTime,endTime),log:maybeLog(result),context:maybeContext(result)})}};var fail=function fail(e){return publish({type:TestEvent.types.TEST,status:TestEvent.status.FAILED,batchId:batchId,testId:assertion.id,behavior:assertion.behavior,error:e})};try{if(assertion.skipped){return publish({type:TestEvent.types.TEST,status:TestEvent.status.SKIPPED,batchId:batchId,testId:assertion.id,behavior:assertion.behavior})}var startTime;return publish({type:TestEvent.types.START_TEST,batchId:batchId,testId:assertion.id,behavior:assertion.behavior}).then(function(){startTime=clock()}).then(function(){return test()}).then(function(result){return pass(startTime)(result)}).catch(fail)}catch(e){return fail(e)}}// /assertOne
/**
...
return behavior.substring(7).trim()}else{return behavior.trim()}};var concatBehavior=function concatBehavior(behavior,key){if(typeof key==="string"&&key.trim().length){return"".concat(trimBehavior(behavior),", ").concat(trimBehavior(key))}return trimBehavior(behavior)};function Layer(input){var id=input.id,behavior=input.behavior,node=input.node,timeout=input.timeout,assertionLibrary=input.assertionLibrary;var parentSkipped=input.skipped;var parentGiven=input.given;var parentWhen=input.when;var parentWhenIsInheritedGiven=input.whenIsInheritedGiven;var skipped=parentSkipped||isSkipped(behavior);var assertions=getAssertions(behavior,node,skipped,timeout);var given=getGiven(node)||parentGiven;var when=getWhen(node);var whenIsInheritedGiven=parentWhenIsInheritedGiven||false;// false is the default
...
/*
          ...
          */whenIsInheritedGiven=true;when=given}return{id:id||makeTestId(behavior),behavior:behavior,given:given,when:when,assertions:assertions,skipped:skipped,timeout:timeout,assertionLibrary:assertionLibrary,whenIsInheritedGiven:whenIsInheritedGiven}}function FlattenedTests(input){var behavior=input.behavior,node=input.node,given=input.given,when=input.when,whenIsInheritedGiven=input.whenIsInheritedGiven,skipped=input.skipped,timeout=input.timeout,assertionLibrary=input.assertionLibrary;var layers=[];var parent=new Layer({id:makeBatchId(behavior),behavior:behavior,node:node,given:given,when:when,whenIsInheritedGiven:whenIsInheritedGiven,skipped:skipped||node.skipped,timeout:timeout||node.timeout,assertionLibrary:assertionLibrary||node.assertionLibrary});if(Array.isArray(parent.assertions)&&parent.assertions.length){layers.push(parent)}Object.keys(node).filter(function(childKey){return _typeof(node[childKey])==="object"}).map(function(childKey){var childBehavior=concatBehavior(behavior,childKey);return FlattenedTests({behavior:childBehavior,node:node[childKey],given:parent.given,when:parent.when,whenIsInheritedGiven:parent.whenIsInheritedGiven,// skipping favors the parent over the child
skipped:parent.skipped||isSkipped(childKey),// timeout and assertion lib favor the child over the parent
timeout:node[childKey].timeout||parent.timeout,assertionLibrary:node[childKey].assertionLibrary||parent.assertionLibrary})}).forEach(function(mappedLayers){mappedLayers.filter(function(mappedLayer){return Array.isArray(mappedLayer.assertions)&&mappedLayer.assertions.length}).forEach(function(mappedLayer){layers.push(mappedLayer)})});return layers}var makeBatch=function makeBatch(tests){return Object.keys(tests).reduce(function(batch,key){return batch.concat(new FlattenedTests({behavior:key,node:tests[key]}))},[])};return{makeBatch:makeBatch}}// /BatchComposer
return{BatchComposer:BatchComposer}}// /factory
};// /exports
module.exports={name:"makeSuiteConfig",factory:function factory(dependencies){'use strict';var defaults=dependencies.defaults,subscribe=dependencies.subscribe,subscriptionExists=dependencies.subscriptionExists,allSubscriptions=dependencies.allSubscriptions,reporterFactory=dependencies.reporterFactory;var makeSuiteId=function makeSuiteId(){return"S".concat((Math.random()*16777215<<0).toString(16).toUpperCase())};var makeSuiteConfig=function makeSuiteConfig(options){var suiteConfig={assertionLibrary:defaults.assertionLibrary,match:defaults.match,name:makeSuiteId(),timeout:2000,reporters:[],givenSynonyms:["given","arrange"],whenSynonyms:["when","act","topic"]};options=_objectSpread({},options);if(options.assertionLibrary){suiteConfig.assertionLibrary=options.assertionLibrary}if(typeof options.match==="string"){suiteConfig.match=new RegExp(options.match)}else if(options.match&&typeof options.match.test==="function"){suiteConfig.match=options.match}else if(options.match===null){// let hard coded options override (I use this in the tests)
suiteConfig.match=options.match}if(typeof options.name==="string"&&options.name.trim().length){suiteConfig.name=options.name.trim()}if(typeof options.timeout==="number"&&options.timeout>0){suiteConfig.timeout=options.timeout}if(typeof options.useColors==="boolean"){suiteConfig.useColors=options.useColors}if(Array.isArray(options.givenSynonyms)){var synonyms=options.givenSynonyms.filter(function(synonym){return typeof synonym==="string"&&synonym.trim().length}).map(function(synonym){return synonym.trim()});if(synonyms.length){suiteConfig.givenSynonyms=synonyms}}if(Array.isArray(options.whenSynonyms)){var _synonyms=options.whenSynonyms.filter(function(synonym){return typeof synonym==="string"&&synonym.trim().length}).map(function(synonym){return synonym.trim()});if(_synonyms.length){suiteConfig.whenSynonyms=_synonyms}}var makeReporterArray=function makeReporterArray(input){return input.split(",").map(function(reporter){return reporter.trim().toUpperCase()})};var addReporter=function addReporter(nameOrFunc){if(typeof nameOrFunc==="string"){var reporter=reporterFactory.get(nameOrFunc);if(!subscriptionExists(reporter.name)){subscribe(reporter)}suiteConfig.reporters.push(reporter)}else{reporterFactory.add(nameOrFunc);addReporter(nameOrFunc.name)}};// reporters: accept strings, functions, or objects
...
var matcher=function matcher(config){return function(theory){if(!config.match){return true}for(var i=0;i<theory.assertions.length;i+=1){if(config.match.test(theory.assertions[i].behavior)){return true}}}};var mapper=function mapper(config,makeBatch,byMatcher){return function(batch){var processed=makeBatch(batch).filter(byMatcher);var batchId=processed.length?processed[0].id:makeBatchId();return{batchId:batchId,batch:processed,tests:processed.map(function(theory){return new AsyncTest(theory,config.makeTheoryConfig(theory),batchId)})}}};var reduceResults=function reduceResults(results){return results.reduce(function(output,current){return Array.isArray(current.value)?output.concat(current.value):output.concat([current.value])},[])};var tester=function tester(config,mapToTests){return function(description,assertions){return normalizeBatch(description,assertions).then(mapToTests).then(function(context){context.plan={count:context.batch.reduce(function(count,item){return count+item.assertions.length},0),completed:0};return context}).then(function(context){if(publishStartAndEnd){return publish({type:TestEvent.types.START,suiteId:config.name}).then(function(){return context})}return Promise.resolve(context)}).then(function(context){var batchId=context.batchId,plan=context.plan;return publish({type:TestEvent.types.START_BATCH,batchId:batchId,suiteId:config.name,plan:plan}).then(function(){return context})}).then(function(context){var batchId=context.batchId,tests=context.tests;return allSettled(tests.map(function(test){return test()})).then(function(results){context.results=results;context.batchTotals=Tally.getTally().batches[batchId];return context})}).then(function(context){var batchId=context.batchId,plan=context.plan,batchTotals=context.batchTotals;return publish({type:TestEvent.types.END_BATCH,batchId:batchId,suiteId:config.name,plan:{count:plan.count,completed:batchTotals.total},totals:batchTotals}).then(function(){return context})}).then(function(context){var batchId=context.batchId,batchTotals=context.batchTotals,results=context.results;var output={batchId:batchId,results:reduceResults(results),totals:batchTotals};if(publishStartAndEnd){return publish({type:TestEvent.types.END_TALLY,suiteId:config.name}).then(function(){return publish({type:TestEvent.types.END,suiteId:config.name,totals:batchTotals})}).then(function(){return output})}return Promise.resolve(output)}).catch(function(e){publish({type:TestEvent.types.TEST,status:TestEvent.status.BROKEN,behavior:"Failed to load test",suiteId:config.name,error:e});throw e})}};var runner=function runner(config,test){return function(findAndRun){return function(){publishStartAndEnd=false;return publish({type:TestEvent.types.START,suiteId:config.name}).then(function(){return findAndRun()}).then(function(output){if(output.broken.length){// these tests failed before being executed
var brokenPromises=output.broken.map(function(error){return publish({type:TestEvent.types.TEST,status:TestEvent.status.BROKEN,behavior:"Failed to load test: ".concat(error.filePath),suiteId:config.name,error:error})});return allSettled(brokenPromises).then(function(){return output})}return Promise.resolve(output)}).then(function(output){return publish({type:TestEvent.types.END_TALLY,suiteId:config.name}).then(function(){return output})}).then(function(output){return publish({type:TestEvent.types.FINAL_TALLY,suiteId:config.name,totals:Tally.getTally()}).then(function(){return output})}).then(function(output){// only get the tally _after_ END_TALLY was emitted
return{output:output,tally:Tally.getSimpleTally()}}).then(function(context){return publish({type:TestEvent.types.END,suiteId:config.name,totals:context.tally}).then(function(){return context})}).then(function(_ref){var output=_ref.output,tally=_ref.tally;return{files:output.files,results:output.results,broken:output.broken,config:output.config,suite:test,totals:tally}})}}};var browserRunner=function browserRunner(config,test){return function(options){return function(){return Array.isArray(options.paths)?runServer(test,options)(options):findFiles(options).then(runServer(test,options))}}};/**
     * The test library
     * @param {Object} suiteConfig : optional configuration
    */function Suite(suiteConfig,envvars){var configure=function configure(_suiteConfig){if(_suiteConfig&&suiteConfig){Object.keys(suiteConfig).forEach(function(key){_suiteConfig[key]=_suiteConfig[key]||suiteConfig[key]})}clearSubscriptions();subscribe(reporterFactory.get(Tally.name));var config=makeSuiteConfig(_suiteConfig);var _ref2=new BatchComposer(config),makeBatch=_ref2.makeBatch;var byMatcher=matcher(config);var mapToTests=mapper(config,makeBatch,byMatcher);var test=tester(config,mapToTests);var findAndStart=browserRunner(config,test);var run=runner(config,test);/**
        // Make a newly configured suite
        */test.id=config.name;// @deprecated
test.printSummary=function(){return publish({type:TestEvent.types.END,suiteId:config.name,totals:Tally.getSimpleTally()})};test.getTotals=function(){return Tally.getSimpleTally()};test.suiteName=config.name;test.runner=function(options){options=options||{};if(envvars&&envvars.file&&typeof envvars.file.test==="function"){options.matchesNamingConvention=envvars.file}return{// find and run (node)
run:run(function(){return findFiles(options).then(resolveTests(test)).then(_runTests(test))}),// run (browser|node)
runTests:function runTests(tests){if(Array.isArray(tests)){options.tests=tests}return run(function(){return _runTests(test)(options)})()},// start test server (browser)
startServer:findAndStart(options)}};test.reporters=config.reporters;test.config=config;test.configure=configure;test.subscribe=function(subscription){subscribe(subscription);return test};test.dependencies=_suiteConfig&&_suiteConfig.inject;test.reporterFactory=reporterFactory;return test};return configure(suiteConfig)}// Suite.suites = []
return{Suite:Suite}}// /factory
};// /module
module.exports={name:"TestEvent",factory:function factory(dependencies){'use strict';var clock=dependencies.clock;var TYPE_EXPRESSION=/(^START$)|(^START_BATCH$)|(^START_TEST$)|(^TEST$)|(^INFO$)|(^END_BATCH$)|(^END_TALLY$)|(^FINAL_TALLY$)|(^END$)/;var STATUS_EXPRESSION=/(^PASSED$)|(^SKIPPED$)|(^FAILED$)|(^BROKEN$)/;var testCount=0;var makeJSONStringifiableError=function makeJSONStringifiableError(err){var error={message:err.message,stack:err.stack};Object.keys(err).forEach(function(key){var _err=err[key];if(_err&&_err.message){error[key]=makeJSONStringifiableError(err[key])}else{error[key]=err[key]}});return error};var TestEvent=function TestEvent(event){var self={};event=Object.assign({},event);self.type=getType(event.type);if(typeof event.time==="number"||typeof event.time==="bigint"){self.time=event.time}else{self.time=clock()}if(event.duration){self.duration=event.duration}if(self.type===TestEvent.types.TEST){testCount+=1;self.count=testCount}if(typeof event.status==="string"&&STATUS_EXPRESSION.test(event.status)){self.status=event.status}else if(event.status){self.status="UNKNOWN"}if(event.behavior){self.behavior=event.behavior}if(event.error){self.error=makeJSONStringifiableError(event.error)}if(event.batchId){self.batchId=event.batchId}if(event.testId){self.testId=event.testId}if(event.suiteId){self.suiteId=event.suiteId}if(event.plan){self.plan=event.plan}if(typeof event.log!=="undefined"){self.log=event.log}if(event.tally){self.tally=event.tally}if(event.totals){self.totals=event.totals}if(event.context){self.context=event.context}return Object.freeze(self)};TestEvent.types={START:"START",START_BATCH:"START_BATCH",START_TEST:"START_TEST",TEST:"TEST",INFO:"INFO",END_BATCH:"END_BATCH",END_TALLY:"END_TALLY",FINAL_TALLY:"FINAL_TALLY",END:"END"};TestEvent.status={PASSED:"PASSED",SKIPPED:"SKIPPED",FAILED:"FAILED",BROKEN:"BROKEN"};function getType(type){if(TYPE_EXPRESSION.test(type)){return type}return"UNKNOWN"}return{TestEvent:TestEvent}}// /factory
};// /module
module.exports={name:"time",factory:function factory(){'use strict';var UNITS={SECONDS:"s",MILLISECONDS:"ms",MICROSECONDS:"us",NANOSECONDS:"ns"};var UNITS_ARRAY=Object.keys(UNITS).map(function(key){return UNITS[key]});var isValidUnit=function isValidUnit(unit){return UNITS_ARRAY.includes(unit)};var makeClock=function makeClock(MULTIPLIERS,makeTime){var clock=function clock(option){switch(option){case"s":return makeTime(MULTIPLIERS.SECONDS);case"ms":return makeTime(MULTIPLIERS.MILLISECONDS);case"us":return makeTime(MULTIPLIERS.MICROSECONDS);case"ns":return makeTime(MULTIPLIERS.NANOSECONDS);default:return{seconds:makeTime(MULTIPLIERS.SECONDS),milliseconds:makeTime(MULTIPLIERS.MILLISECONDS),microseconds:makeTime(MULTIPLIERS.MICROSECONDS),nanoseconds:makeTime(MULTIPLIERS.NANOSECONDS)};}};clock.seconds=function(){return clock(UNITS.SECONDS)};clock.milliseconds=function(){return clock(UNITS.MILLISECONDS)};clock.microseconds=function(){return clock(UNITS.MICROSECONDS)};clock.nanoseconds=function(){return clock(UNITS.NANOSECONDS)};return clock};var CONVERSIONS={s:{SECONDS:1,MILLISECONDS:1000,MICROSECONDS:1000000,NANOSECONDS:1000000000},ms:{SECONDS:0.001,MILLISECONDS:1,MICROSECONDS:1000,NANOSECONDS:1000000},us:{SECONDS:0.000001,MILLISECONDS:0.001,MICROSECONDS:1,NANOSECONDS:1000},ns:{SECONDS:1e9,MILLISECONDS:0.000001,MICROSECONDS:0.001,NANOSECONDS:1}};var duration=function duration(start,end,timeUnits){var conversions=CONVERSIONS[timeUnits];return{seconds:(end-start)*conversions.SECONDS,milliseconds:(end-start)*conversions.MILLISECONDS,microseconds:(end-start)*conversions.MICROSECONDS,nanoseconds:(end-start)*conversions.NANOSECONDS}};var NODE_MULTIPLIERS={SECONDS:[1,1e-9],MILLISECONDS:[1e3,1e-6],MICROSECONDS:[1e6,1e-3],NANOSECONDS:[1e9,1]};var BROWSER_MULTIPLIERS={SECONDS:0.001,MILLISECONDS:1,MICROSECONDS:1000,NANOSECONDS:1000000};var nodeClock=function nodeClock(multipliers,hrtime){var time=Array.isArray(hrtime)?process.hrtime(hrtime):process.hrtime();return time[0]*multipliers[0]+time[1]*multipliers[1]};var browserClock=function browserClock(multiplier,hrtime){return window.performance.now()*multiplier};var clock=typeof process!=="undefined"&&typeof process.hrtime==="function"?makeClock(NODE_MULTIPLIERS,nodeClock):makeClock(BROWSER_MULTIPLIERS,browserClock);return{clock:clock,isValidUnit:isValidUnit,duration:duration}}};module.exports={name:"runTests",factory:function factory(dependencies){'use strict';var allSettled=dependencies.allSettled;var hasThen=function hasThen(obj){return obj&&typeof obj.then==="function"};var toPromise=function toPromise(config,suite){return function(input){var err,test,path;try{if(typeof input==="function"){test=input}else if(input){err=input.err;test=input.test;path=input.path}else{throw new Error("Invalid runTests entry: expected input ".concat(_typeof(input)," to be a {function}, or { err?: Error; test: Function|Promise; path?: string; }"))}if(err){err.filePath=path;return Promise.reject(err)}if(hasThen(test)){// module.exports = test('something', (t) => {...})
...
return{DefaultFormatter:DefaultFormatter}}};module.exports={name:"JsonFormatter",factory:function factory(dependencies){'use strict';var TestEvent=dependencies.TestEvent;function JsonFormatter(){var format=function format(event){if(event.type===TestEvent.types.START){return"[".concat(JSON.stringify({event:event},null,2),",")}else if(event.type===TestEvent.types.END){return"".concat(JSON.stringify({event:event},null,2),"]")}else if([TestEvent.types.START_TEST,TestEvent.types.END_TALLY].indexOf(event.type)===-1){return"".concat(JSON.stringify({event:event},null,2),",")}};return{format:format}}return{JsonFormatter:JsonFormatter}}};module.exports={name:"MarkdownFormatter",factory:function factory(dependencies){'use strict';var consoleStyles=dependencies.consoleStyles,TestEvent=dependencies.TestEvent,SpecFormatter=dependencies.SpecFormatter,DefaultFormatter=dependencies.DefaultFormatter;var newLine=consoleStyles.newLine();var _ref3=new SpecFormatter,makeOrderId=_ref3.makeOrderId,makeSpec=_ref3.makeSpec;var _ref4=new DefaultFormatter,formatDuration=_ref4.formatDuration;var toBullets=function toBullets(md,SPACE,layer){if(typeof layer==="undefined")layer=0;var output="";Object.keys(md).forEach(function(key){var line=key.replace(/\n/g,newLine+SPACE.substring(0,(layer+1)*2));output+=SPACE.substring(0,layer*2)+"* ".concat(line).concat(newLine);if(md[key].type&&md[key].type===TestEvent.types.TEST){// done
}else{output+=toBullets(md[key],SPACE,layer+1)}});return output};var makeTotalsH2=function makeTotalsH2(totals){return"## Totals".concat(newLine)+"".concat(newLine,"- **total**: ").concat(totals.total)+"".concat(newLine,"- **passed**: ").concat(totals.passed)+"".concat(newLine,"- **failed**: ").concat(totals.failed)+"".concat(newLine,"- **skipped**: ").concat(totals.skipped)+"".concat(newLine,"- **duration**: ").concat(formatDuration(totals.duration))};var makeErrorsH2=function makeErrorsH2(specs){var found=specs.filter(function(event){return event.status===TestEvent.status.FAILED||event.status===TestEvent.status.BROKEN});if(!found.length){return}var output="## Errors".concat(newLine);found.forEach(function(event){if(event.error&&event.error.stack){output+="".concat(newLine).concat(event.behavior).concat(newLine)+"```"+"".concat(newLine).concat(event.error.stack).concat(newLine)+"```"+newLine}else{output+="".concat(newLine).concat(event.behavior).concat(newLine)}});return output};function MarkdownFormatter(){var title="Tests";var order=[];var specs=[];var format=function format(event){if(event.type===TestEvent.types.START){title=event.suiteId}else if(event.type===TestEvent.types.END){var _makeSpec=makeSpec(order,specs),spec=_makeSpec.spec,SPACE=_makeSpec.SPACE;var errors=makeErrorsH2(specs);if(errors){return"# ".concat(title).concat(newLine).concat(newLine).concat(toBullets(spec,SPACE)).concat(newLine).concat(errors).concat(newLine).concat(makeTotalsH2(event.totals))}else{return"# ".concat(title).concat(newLine).concat(newLine).concat(toBullets(spec,SPACE)).concat(newLine).concat(makeTotalsH2(event.totals))}}else if(event.type===TestEvent.types.START_TEST){order.push(makeOrderId(event))}else if(event.type===TestEvent.types.TEST){specs.push(event)}};// /format
return{format:format}}// /Formatter
return{MarkdownFormatter:MarkdownFormatter}}};module.exports={name:"SummaryFormatter",factory:function factory(dependencies){'use strict';var consoleStyles=dependencies.consoleStyles,DefaultFormatter=dependencies.DefaultFormatter,TestEvent=dependencies.TestEvent;function SummaryFormatter(){var defaultFormat=DefaultFormatter({// Other than INFO, these aren't actually used, since this doesn't produce TEST output
...
function isPromise(input){return input&&typeof input.then==="function"}var time=module.factories.timeFactory();var suites={};var supposed=null;// resolve the dependency graph
function Supposed(options){var _module$factories$all=module.factories.allSettledFactory({}),allSettled=_module$factories$all.allSettled;var _module$factories$run=module.factories.runTestsFactory({allSettled:allSettled}),runTests=_module$factories$run.runTests;var envvars={assertionLibrary:{},reporters:["LIST"],useColors:true,timeUnits:"us"};var clock=function clock(){return time.clock(envvars.timeUnits)};var duration=function duration(start,end){return time.duration(start,end,envvars.timeUnits)};var _module$factories$Tes=module.factories.TestEventFactory({clock:clock}),TestEvent=_module$factories$Tes.TestEvent;var _module$factories$pub=module.factories.pubsubFactory({allSettled:allSettled,isPromise:isPromise,TestEvent:TestEvent}),Pubsub=_module$factories$pub.Pubsub;var _ref5=new Pubsub,publish=_ref5.publish,subscribe=_ref5.subscribe,subscriptionExists=_ref5.subscriptionExists,allSubscriptions=_ref5.allSubscriptions,reset=_ref5.reset;var consoleStyles=module.factories.consoleStylesFactory({envvars:envvars}).consoleStyles;var _module$factories$Tal=module.factories.TallyFactory({publish:publish,TestEvent:TestEvent,clock:clock,duration:duration}),TallyFactory=_module$factories$Tal.TallyFactory;var _TallyFactory=TallyFactory({}),Tally=_TallyFactory.Tally;var _module$factories$rep=module.factories.reporterFactoryFactory({}),ReporterFactory=_module$factories$rep.ReporterFactory;var reporterFactory=new ReporterFactory;var ArrayReporter=module.factories.ArrayReporterFactory({}).ArrayReporter;reporterFactory.add(ArrayReporter);reporterFactory.add(function QuietReporter(){// legacy
return{write:new ArrayReporter().write}});reporterFactory.add(module.factories.NoopReporterFactory({}).NoopReporter);reporterFactory.add(Tally);function DefaultFormatter(options){return module.factories.DefaultFormatterFactory({consoleStyles:consoleStyles,TestEvent:TestEvent,SYMBOLS:options&&options.SYMBOLS||{PASSED:" PASS ",FAILED:" FAIL ",BROKEN:" !!!! ",SKIPPED:" SKIP ",INFO:" INFO "}}).DefaultFormatter()}function ConsoleReporter(options){return module.factories.DomReporterFactory({TestEvent:TestEvent,formatter:options.formatter}).DomReporter()}var listFormatter=module.factories.ListFormatterFactory({consoleStyles:consoleStyles,DefaultFormatter:DefaultFormatter}).ListFormatter();var SpecFormatter=module.factories.SpecFormatterFactory({consoleStyles:consoleStyles,DefaultFormatter:DefaultFormatter,TestEvent:TestEvent}).SpecFormatter;reporterFactory.add(function ListReporter(){return{write:ConsoleReporter({formatter:listFormatter}).write}}).add(function BlockReporter(){return{write:ConsoleReporter({formatter:module.factories.BlockFormatterFactory({consoleStyles:consoleStyles,DefaultFormatter:DefaultFormatter}).BlockFormatter()}).write}}).add(function BriefReporter(){return{write:ConsoleReporter({formatter:module.factories.BriefFormatterFactory({consoleStyles:consoleStyles,DefaultFormatter:DefaultFormatter,TestEvent:TestEvent}).BriefFormatter()}).write}}).add(function JsonReporter(){return{write:ConsoleReporter({formatter:module.factories.JsonFormatterFactory({TestEvent:TestEvent}).JsonFormatter()}).write}}).add(function MarkdownReporter(){return{write:ConsoleReporter({formatter:module.factories.MarkdownFormatterFactory({consoleStyles:consoleStyles,TestEvent:TestEvent,SpecFormatter:SpecFormatter,DefaultFormatter:DefaultFormatter}).MarkdownFormatter()}).write}}).add(function JustTheDescriptionsReporter(){return{write:ConsoleReporter({formatter:{format:function format(event){if(event.type===TestEvent.types.TEST){return listFormatter.format(event).split("\n")[0]}else{return listFormatter.format(event)}}}}).write}}).add(function SpecReporter(){return{write:ConsoleReporter({formatter:SpecFormatter()}).write}}).add(function SummaryReporter(){return{write:ConsoleReporter({formatter:module.factories.SummaryFormatterFactory({consoleStyles:consoleStyles,DefaultFormatter:DefaultFormatter,TestEvent:TestEvent}).SummaryFormatter()}).write}}).add(function TapReporter(){return{write:ConsoleReporter({formatter:module.factories.TapFormatterFactory({consoleStyles:consoleStyles,TestEvent:TestEvent}).TapFormatter()}).write}});var _module$factories$Asy=module.factories.AsyncTestFactory({isPromise:isPromise,publish:publish,TestEvent:TestEvent,clock:clock,duration:duration}),AsyncTest=_module$factories$Asy.AsyncTest;var _module$factories$has=module.factories.hashFactory(),hash=_module$factories$has.hash;var _module$factories$mak=module.factories.makeBatchFactory({hash:hash}),BatchComposer=_module$factories$mak.BatchComposer;var _module$factories$mak2=module.factories.makeSuiteConfigFactory({defaults:envvars,subscriptionExists:subscriptionExists,subscribe:subscribe,allSubscriptions:allSubscriptions,reporterFactory:reporterFactory}),makeSuiteConfig=_module$factories$mak2.makeSuiteConfig;var _module$factories$Sui=module.factories.SuiteFactory({allSettled:allSettled,AsyncTest:AsyncTest,BatchComposer:BatchComposer,makeSuiteConfig:makeSuiteConfig,publish:publish,subscribe:subscribe,clearSubscriptions:reset,reporterFactory:reporterFactory,runTests:runTests,Tally:Tally,TestEvent:TestEvent}),Suite=_module$factories$Sui.Suite;var suite=new Suite(options);suite.Suite=Supposed;if(!suites[suite.config.name]){suites[suite.config.name]=suite}return suite}supposed=Supposed({name:"supposed"});suites.supposed=supposed;supposed.suites=suites;supposed.time=time;window.supposed=supposed})(window);