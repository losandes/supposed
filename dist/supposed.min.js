...
var _maybePromise=test.default(suite,suite.dependencies);return hasThen(_maybePromise)?_maybePromise:Promise.resolve(_maybePromise)}return Promise.resolve()}catch(e){e.filePath=path;return Promise.reject(e)}}};var mapToResults=function mapToResults(runConfig){var paths=arguments.length>1&&arguments[1]!==undefined?arguments[1]:[];return function(results){return{files:paths,runConfig:runConfig,broken:results.filter(function(result){return result.status!=="fullfilled"}).map(function(result){return result.reason})}}};var makePlans=function makePlans(suite){return function(context){var runConfig=context.runConfig,tests=context.tests,paths=context.paths;if(!tests){throw new Error("run-tests expects tests to be provided")}return Promise.resolve(tests.map(toPromise(runConfig||context,suite))).then(allSettled).then(mapToResults(runConfig,paths))}};return{makePlans:makePlans}}// /factory
};// /module
