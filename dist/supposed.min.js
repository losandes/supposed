...
return 0}}};function ConsoleReporter(options){options=_objectSpread(_objectSpread(_objectSpread({},{reportOrder:REPORT_ORDERS.NON_DETERMINISTIC}),envvars),options);var testEvents=[];var writeOne=function writeOne(event){var line=format(event);if(line){console.log(line)}};var write=function write(event){if(event.type===TestEvent.types.START){writeOne(event)}else if(event.type===TestEvent.types.END){if(options.reportOrder===REPORT_ORDERS.NON_DETERMINISTIC){writeOne(event)}else{writeOne({isDeterministicOutput:true,testEvents:testEvents.sort(byPlanOrder(event.plan.order)),endEvent:event})}}else if(event.type===TestEvent.types.START_TEST){writeOne(event)}else if(event.type===TestEvent.types.TEST){if(options.reportOrder===REPORT_ORDERS.NON_DETERMINISTIC){writeOne(event)}else{testEvents.push(event)}}};// /write
