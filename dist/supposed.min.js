...
resolve({status:"rejected",reason:err})}})}))}return{allSettled:allSettled}}};module.exports={name:"AsyncTest",factory:function factory(dependencies){'use strict';var isPromise=dependencies.isPromise,publish=dependencies.publish,TestEvent=dependencies.TestEvent,clock=dependencies.clock,duration=dependencies.duration,addDurations=dependencies.addDurations;function noop(){}/**
...
test.assertions.filter(function(a){return a.skipped}).length===test.assertions.length}/**
     ...
     * @param {Object} context
     */function runGiven(context){if(typeof context.given!=="function"&&_typeof(context.given)!=="object"){return Promise.resolve(context)}try{var startTime=clock();var actual=context.given();if(isPromise(actual)){return actual.then(function(value){context.resultOfGiven=value;return context}).catch(function(e){context.err=e;throw e})}context.givenDuration=duration(startTime,clock());context.resultOfGiven=actual;return Promise.resolve(context)}catch(e){context.err=e;throw e}}/**
     * Runs `when` and passes any output forward
     * @param {Object} context
     */function runWhen(context){if(typeof context.when!=="function"&&_typeof(context.when)!=="object"){return Promise.resolve(context)}try{var startTime=clock();var actual=context.when(context.resultOfGiven);if(isPromise(actual)){return actual.then(function(value){context.resultOfWhen=value;return context}).catch(function(e){context.err=e;return context})}context.whenDuration=duration(startTime,clock());context.resultOfWhen=actual;return Promise.resolve(context)}catch(e){context.err=e;return context}}/**
     * Executes the assertions
     * @param {Object} context
     */function checkAssertions(context){var promises=context.test.assertions.map(function(assertion){return assertOne(context,assertion,function(){if(assertion.test.length>1){// the assertion accepts all arguments to a single function
