...
/**
     ...
     *     tests: IAsyncTest[]
     *   }
     */var mapper=function mapper(config,makeBatch,byMatcher){return function(batch){var theories=makeBatch(batch).filter(byMatcher);return{batchId:theories.length?theories[0].id:makeBatchId(),theories:theories}}};var planner=function planner(config,mapToBatch){var plan={count:0,completed:0,batches:[]};var addToPlan=function addToPlan(description,assertions){return normalizeBatch(description,assertions).then(mapToBatch).then(function(context){if(context.theories.length){plan.batches.push(context);plan.count+=context.theories.reduce(function(count,item){return count+item.assertions.length},0)}return plan})};addToPlan.getPlan=function(){return plan};return addToPlan};var brokenTestPublisher=function brokenTestPublisher(suiteId){return function(error){return publish({type:TestEvent.types.TEST,status:TestEvent.status.BROKEN,behavior:"Failed to load test: ".concat(error.filePath),suiteId:suiteId,error:error})}};/**
     ...
     */var toOneArray=function toOneArray(prop){return function(output,current){return Array.isArray(current[prop])?output.concat(current[prop]):output.concat([current[prop]])}};var fullfilledToOneArray=toOneArray("value");var failedToOneArray=toOneArray("reason");var batchRunner=function batchRunner(config,publishOneBrokenTest){return function(batch,plan){return publish({type:TestEvent.types.START_BATCH,batchId:batch.batchId,suiteId:config.name,plan:plan}).then(function(){// map the batch theories to tests
return batch.theories.map(function(theory){return new AsyncTest(theory,config.makeTheoryConfig(theory),batch.batchId,config.name)})}).then(function(tests){return allSettled(tests.map(function(test){return test()}))}).then(function(results){return{results:results.filter(function(result){return result.status==="fullfilled"}).reduce(fullfilledToOneArray,[]),broken:results.filter(function(result){return result.status!=="fullfilled"}).reduce(failedToOneArray,[]),batchTotals:Tally.getTally().batches[batch.batchId]}}).then(function(context){var publishEndBatch=function publishEndBatch(){return publish({type:TestEvent.types.END_BATCH,batchId:batch.batchId,suiteId:config.name,totals:context.batchTotals})};if(Array.isArray(context.broken)&&context.broken.length){// these tests failed during the planning stage
...
var tester=function tester(config,runBatch,runnerMode){return function(plan){return Promise.resolve({plan:plan}).then(function(context){if(!runnerMode){return publish({type:TestEvent.types.START,suiteId:config.name,plan:{count:context.plan.count,completed:0}}).then(function(){return context})}return Promise.resolve(context)}).then(function(context){return Promise.all(context.plan.batches.map(function(batch){return runBatch(batch,context.plan)}))}).then(function(context){if(!runnerMode){return publish({type:TestEvent.types.END_TALLY,suiteId:config.name}).then(function(){return publish({type:TestEvent.types.END,suiteId:config.name,totals:Tally.getSimpleTally()})}).then(function(){return context})}return Promise.resolve(context)}).then(function(context){if(Array.isArray(context)&&context.length===1){return context[0]}return context}).catch(function(e){publish({type:TestEvent.types.TEST,status:TestEvent.status.BROKEN,behavior:"Failed to load test",suiteId:config.name,error:e});throw e})}};var runner=function runner(config,suite,publishOneBrokenTest,execute){return function(planContext){var plan=planContext.plan,files=planContext.files,broken=planContext.broken;return publish({type:TestEvent.types.START,suiteId:config.name,plan:{count:plan.count,completed:0}}).then(function(){if(broken&&broken.length){// these tests failed during the planning stage
return allSettled(broken.map(publishOneBrokenTest))}}).then(function(){return execute(plan)}).then(function(output){return publish({type:TestEvent.types.END_TALLY,suiteId:config.name}).then(function(){return output})}// pass through
).then(function(output){return publish({type:TestEvent.types.FINAL_TALLY,suiteId:config.name,totals:Tally.getTally()}).then(function(){return output})}// pass through
).then(function(output){// only get the tally _after_ END_TALLY was emitted
return{output:output,tally:Tally.getSimpleTally()}}).then(function(context){return publish({type:TestEvent.types.END,suiteId:config.name,totals:context.tally}).then(function(){return context})}// pass through
).then(function(_ref){var output=_ref.output,tally=_ref.tally;return{files:files,results:output.results,broken:broken,config:planContext.config,suite:suite,totals:tally}})}};var browserRunner=function browserRunner(config,test){return function(options){return function(){return Array.isArray(options.paths)?runServer(test,options)(options):findFiles(options).then(runServer(test,options))}}};/**
     * The test library
     * @param {Object} suiteConfig : optional configuration
    */function Suite(suiteConfig,envvars){var runnerMode=false;suiteConfig=_objectSpread({},suiteConfig);/**
       * @param suiteDotConfigureOptions - configuration provided in line with `supposed.Suite().configure(suiteDotConfigureOptions)`
       */var configure=function configure(suiteDotConfigureOptions){suiteDotConfigureOptions=_objectSpread({},suiteDotConfigureOptions);var _suiteConfig=Object.keys(suiteConfig).concat(Object.keys(suiteDotConfigureOptions)).reduce(function(cfg,key){cfg[key]=typeof suiteDotConfigureOptions[key]!=="undefined"?suiteDotConfigureOptions[key]:suiteConfig[key];return cfg},{});clearSubscriptions();subscribe(reporterFactory.get(Tally.name));var config=makeSuiteConfig(_suiteConfig);var publishOneBrokenTest=brokenTestPublisher(config.name);var _ref2=new BatchComposer(config),makeBatch=_ref2.makeBatch;var byMatcher=matcher(config);var mapToBatch=mapper(config,makeBatch,byMatcher);var runBatch=batchRunner(config,publishOneBrokenTest);var plan=planner(config,mapToBatch);var test=function test(description,assertions){if(runnerMode){return plan(description,assertions)}else{return plan(description,assertions).then(tester(config,runBatch,runnerMode))}};test.id=config.name;test.plan=plan;test.reporters=config.reporters;test.config=config;test.dependencies=_suiteConfig&&_suiteConfig.inject;test.configure=configure;test.subscribe=function(subscription){subscribe(subscription);return test};test.runner=function(options){options=options||{};if(envvars&&envvars.file&&typeof envvars.file.test==="function"){options.matchesNamingConvention=envvars.file}var findAndStart=browserRunner(config,test);var addPlanToContext=function addPlanToContext(){return function(context){context.plan=plan.getPlan();return context}};var findAndPlan=function findAndPlan(){runnerMode=true;return findFiles(options).then(resolveTests()).then(makePlans(test)).then(addPlanToContext())};return{plan:findAndPlan,// find and run (node)
run:function run(){return findAndPlan().then(runner(config,test,publishOneBrokenTest,tester(config,runBatch,runnerMode)))},// run (browser|node)
runTests:function runTests(tests){if(Array.isArray(tests)){options.tests=tests}runnerMode=true;return makePlans(test)(options).then(addPlanToContext()).then(runner(config,test,publishOneBrokenTest,tester(config,runBatch,runnerMode)))},// start test server (browser)
startServer:findAndStart(options)}};// @deprecated - may go away in the future
test.printSummary=function(){return publish({type:TestEvent.types.END,suiteId:config.name,totals:Tally.getSimpleTally()})};// @deprecated - may go away in the future
test.getTotals=function(){return Tally.getSimpleTally()};return test};return configure()}// Suite.suites = []
return{Suite:Suite}}// /factory
};// /module
module.exports={name:"TestEvent",factory:function factory(dependencies){'use strict';var clock=dependencies.clock;var TYPE_EXPRESSION=/(^START$)|(^START_BATCH$)|(^START_TEST$)|(^TEST$)|(^END_BATCH$)|(^END_TALLY$)|(^FINAL_TALLY$)|(^END$)/;var STATUS_EXPRESSION=/(^PASSED$)|(^SKIPPED$)|(^FAILED$)|(^BROKEN$)/;var testCount=0;var makeJSONStringifiableError=function makeJSONStringifiableError(err){var error={message:err.message,stack:err.stack};Object.keys(err).forEach(function(key){var _err=err[key];if(_err&&_err.message){error[key]=makeJSONStringifiableError(err[key])}else{error[key]=err[key]}});return error};var TestEvent=function TestEvent(event){var self={};event=Object.assign({},event);if(event.suiteId){self.suiteId=event.suiteId}if(event.batchId){self.batchId=event.batchId}if(event.testId){self.testId=event.testId}if(event.type===TestEvent.types.TEST){testCount+=1;self.count=testCount}if(typeof event.time==="number"||typeof event.time==="bigint"){self.time=event.time}else{self.time=clock()}self.type=getType(event.type);if(typeof event.status==="string"&&STATUS_EXPRESSION.test(event.status)){self.status=event.status}else if(event.status){self.status="UNKNOWN"}if(event.behavior){self.behavior=event.behavior}if(Array.isArray(event.behaviors)){self.behaviors=event.behaviors}if(event.plan){self.plan=event.plan}if(event.error){self.error=makeJSONStringifiableError(event.error)}if(typeof event.log!=="undefined"){self.log=event.log}if(event.context){self.context=event.context}if(event.duration){self.duration=event.duration}if(event.tally){self.tally=event.tally}if(event.totals){self.totals=event.totals}return Object.freeze(self)};TestEvent.types={START:"START",START_BATCH:"START_BATCH",START_TEST:"START_TEST",TEST:"TEST",END_BATCH:"END_BATCH",END_TALLY:"END_TALLY",FINAL_TALLY:"FINAL_TALLY",END:"END"};TestEvent.status={PASSED:"PASSED",SKIPPED:"SKIPPED",FAILED:"FAILED",BROKEN:"BROKEN"};function getType(type){if(TYPE_EXPRESSION.test(type)){return type}return"UNKNOWN"}return{TestEvent:TestEvent}}// /factory
};// /module
...
var _maybePromise=test.default(suite,suite.dependencies);return hasThen(_maybePromise)?_maybePromise:Promise.resolve(_maybePromise)}return Promise.resolve()}catch(e){e.filePath=path;return Promise.reject(e)}}};var mapToResults=function mapToResults(config){var paths=arguments.length>1&&arguments[1]!==undefined?arguments[1]:[];return function(results){return{files:paths,config:config,broken:results.filter(function(result){return result.status!=="fullfilled"}).map(function(result){return result.reason})}}};var makePlans=function makePlans(suite){return function(context){var config=context.config,tests=context.tests,paths=context.paths;if(!tests){throw new Error("run-tests expects tests to be provided")}return Promise.resolve(tests.map(toPromise(config||context,suite))).then(allSettled).then(mapToResults(config,paths))}};return{makePlans:makePlans}}// /factory
};// /module
...
FAILED:consoleStyles.red("\u2717 "),// heavy-x '✘',
BROKEN:consoleStyles.red("!= "),// heavy-x '✘',
SKIPPED:consoleStyles.yellow("\u2E15 "),INFO:consoleStyles.cyan("# ")};var _DefaultFormatter=DefaultFormatter({SYMBOLS:SYMBOLS}),format=_DefaultFormatter.format,formatInfo=_DefaultFormatter.formatInfo,formatExpectedAndActual=_DefaultFormatter.formatExpectedAndActual,formatStack=_DefaultFormatter.formatStack;var addToSpec=function addToSpec(parts,spec,event){if(!parts.length){return}var part=parts.shift();if(parts.length){spec[part]=spec[part]||{};addToSpec(parts,spec[part],event)}else{spec[part]=event}};var toPrint=function toPrint(spec,SPACE,layer){if(typeof layer==="undefined")layer=0;var output="";// use getOwnPropertyNames instead of keys because the order is guarnateed back to es2015
...
}).write}}).add(function SummaryReporter(){return{write:ConsoleReporter({formatter:module.factories.SummaryFormatterFactory({consoleStyles:consoleStyles,DefaultFormatter:DefaultFormatter,TestEvent:TestEvent}).SummaryFormatter()}).write}}).add(function TapReporter(){return{write:ConsoleReporter({formatter:module.factories.TapFormatterFactory({consoleStyles:consoleStyles,TestEvent:TestEvent}).TapFormatter()}).write}});var _module$factories$Asy=module.factories.AsyncTestFactory({isPromise:isPromise,publish:publish,TestEvent:TestEvent,clock:clock,duration:duration,addDurations:time.addDurations}),AsyncTest=_module$factories$Asy.AsyncTest;var _module$factories$has=module.factories.hashFactory(),hash=_module$factories$has.hash;var _module$factories$mak2=module.factories.makeBatchFactory({hash:hash}),BatchComposer=_module$factories$mak2.BatchComposer;var _module$factories$mak3=module.factories.makeSuiteConfigFactory({defaults:envvars,subscriptionExists:subscriptionExists,subscribe:subscribe,allSubscriptions:allSubscriptions,reporterFactory:reporterFactory}),makeSuiteConfig=_module$factories$mak3.makeSuiteConfig;var _module$factories$Sui=module.factories.SuiteFactory({allSettled:allSettled,AsyncTest:AsyncTest,BatchComposer:BatchComposer,makeSuiteConfig:makeSuiteConfig,publish:publish,subscribe:subscribe,clearSubscriptions:reset,reporterFactory:reporterFactory,makePlans:makePlans,Tally:Tally,TestEvent:TestEvent}),Suite=_module$factories$Sui.Suite;var suite=new Suite(options);suite.Suite=Supposed;if(!suites[suite.config.name]){suites[suite.config.name]=suite}return suite}supposed=Supposed({name:"supposed"});suites.supposed=supposed;supposed.suites=suites;supposed.time=time;window.supposed=supposed})(window);