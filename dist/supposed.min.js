...
/**
     ...
     *     tests: IAsyncTest[]
     *   }
     */var mapper=function mapper(config,makeBatch,byMatcher){return function(batch){var theories=makeBatch(batch).filter(byMatcher);return{batchId:theories.length?theories[0].id:makeBatchId(),theories:theories}}};var isPlan=function isPlan(maybePlan){return typeof maybePlan!=="undefined"&&maybePlan!==null&&typeof maybePlan.count==="number"&&typeof maybePlan.complete==="number"&&Array.isArray(maybePlan.batches)&&Array.isArray(maybePlan.order)};var planner=function planner(config,mapToBatch){var plan={count:0,completed:0,batches:[],order:[]};var addToPlan=function addToPlan(description,assertions){return normalizeBatch(description,assertions).then(mapToBatch).then(function(context){if(context.theories.length){plan.batches.push(context);plan.count+=context.theories.reduce(function(count,item){item.assertions.forEach(function(assertion){return plan.order.push(assertion.id)});return count+item.assertions.length},0)}return plan})};addToPlan.getPlan=function(){return plan};return addToPlan};var brokenTestPublisher=function brokenTestPublisher(suiteId){return function(error){return publish({type:TestEvent.types.TEST,status:TestEvent.status.BROKEN,behavior:"Failed to load test: ".concat(error.filePath),suiteId:suiteId,error:error})}};/**
...
return{output:output,tally:Tally.getSimpleTally()}}).then(function(context){return publish({type:TestEvent.types.END,suiteId:config.name,totals:context.tally,plan:plan}).then(function(){return context})}// pass through
...
runTests:function runTests(planOrTests){runnerMode=true;var run=runner(config,test,publishOneBrokenTest,tester(config,runBatch,runnerMode));if(isPlan(planOrTests)){return Promise.resolve({files:Array.isArray(planOrTests.files)?planOrTests.files:[],plan:planOrTests,config:options||{},broken:[]}).then(run)}if(Array.isArray(planOrTests)){options.tests=planOrTests}else if(typeof planOrTests==="function"){options.tests=planOrTests()}return makePlans(test)(options).then(addPlanToContext()).then(run)},// start test server (browser)
...
return 0}}};function ConsoleReporter(options){options=_objectSpread({},{reportOrder:REPORT_ORDERS.NON_DETERMINISTIC},{},envvars,{},options);var testEvents=[];var writeOne=function writeOne(event){var line=format(event);if(line){console.log(line)}};var write=function write(event){if(event.type===TestEvent.types.START){writeOne(event)}else if(event.type===TestEvent.types.END){if(options.reportOrder===REPORT_ORDERS.NON_DETERMINISTIC){writeOne(event)}else{writeOne({isDeterministicOutput:true,testEvents:testEvents.sort(byPlanOrder(event.plan.order)),endEvent:event})}}else if(event.type===TestEvent.types.START_TEST){writeOne(event)}else if(event.type===TestEvent.types.TEST){if(options.reportOrder===REPORT_ORDERS.NON_DETERMINISTIC){writeOne(event)}else{testEvents.push(event)}}};// /write
